...
// create global storage for last frame
//int oldFrame[512] = {}; // will have indices 0-511 // not enough memeory for this one :(
...
/*void sendComparedDMX() {
  for (int i = 0 ; i < artnet.getLength() ; i++) {
    int v = artnet.getDmxFrame()[i];
    if (v != oldFrame[i]) { // check if the value has changed since the last frame
      int c = i+1;
      DmxSimple.write(c, v);
    }
    oldFrame[i] = v; // update oldFrame with the value for use next time
  }
}*/
